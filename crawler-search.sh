...
END_TIME=$(date +%s)
echo "Crawler and Indexer Process finished!!! $(($END_TIME - $BEG_TIME)) seconds"
echo
echo
echo
echo
...
function search_word(){
	echo "Enter the word which u want to search : "
	read input
	echo "SEARCH PROCEDURE"
	echo "Counting occurrences for the word $input in files"
	echo
	...
	echo "want to search a another word occurance?, press 'y' or 'n' "
	read input
...
echo "want to search a word occurance?, press 'y' or 'n' "
read input
if [ $input == "y" ] || [ $input == "Y" ]; then
	search_word
else
	echo "Crawling and indexing done"
	exit 1
fi
exit
